...
多通道、反馈纹理或自行管理参数的场景，可在 [`src/utils/webgl-wallpaper/scenes/`](../src/utils/webgl-wallpaper/scenes/) 新增 `my-scene.ts`。模块默认导出一个接收 `WebGLWallpaperSceneContext` 的工厂函数，返回含 `dispose()` 的对象；把配置 `scene` 改为 `"my-scene"` 即可。宿主通过 `import.meta.glob` 按需加载，不用修改模式切换代码。工厂应在画出第一帧后调用 `onFirstFrame()`，并在 `dispose()` 中取消动画帧、监听器及 GPU 资源；可使用 `signal` 取消异步资产加载。

```ts
import type { WebGLWallpaperSceneContext } from "../types";

export default function createScene({ canvas, onFirstFrame }: WebGLWallpaperSceneContext) {
...
Cloud Train 的完整多通道实现见 [`cloud-train.ts`](../src/utils/webgl-wallpaper/scenes/cloud-train.ts)，宿主生命周期见 [`host.ts`](../src/utils/webgl-wallpaper/host.ts)。
