...
function settings(options: Record<string, unknown>): CloudTrainSettings {
	return {
		...
		resolution: number(options.resolution, 1, 0.25, 1),
		maxRenderWidth: Math.round(number(options.maxRenderWidth, 1920, 640, 3840)),
		maxRenderHeight: Math.round(
			number(options.maxRenderHeight, 1080, 360, 2160),
		),
		...
		detail: Math.round(number(options.detail, 5, 1, 8)),
...
class CloudTrainRenderer implements WebGLWallpaperScene {
	...
	#start(): (() => void) | undefined {
		...
		try {
			// Three foreground samples with the temporal spread reduced to one third.
			const scene = program(
					fragment.replace(
			...
			function updatePixelSize() {
				// Cap the internal framebuffer at a 1080p-class budget while keeping
				// the viewport aspect ratio; devicePixelRatio never scales it up.
				const s = state.current;
				const capScale = Math.min(
					1,
					s.maxRenderWidth / Math.max(cssWidth, 1),
					s.maxRenderHeight / Math.max(cssHeight, 1),
				);
				const d = capScale * s.resolution;
				pixelWidth = Math.max(
					1,
