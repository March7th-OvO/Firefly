...
const timeText = $derived(formatTime(now, use12Hour, showSeconds));
const dateText = $derived(formatDate(now));
const lunarText = $derived(showLunar ? formatLunarDate(now) : "");
...
function formatTime(date: Date, hour12: boolean, seconds: boolean): string {
...
function setWallpaperPreference(preference: DiscoverWallpaperPreference): void {
...
onMount(() => {
	...
	const handleDocumentPointerDown = (event: PointerEvent) => {
		...
		if (!(target instanceof Node) || searchContainer?.contains(target)) return;
		engineMenuOpen = false;
		searchActive = false;
		searchFocused = false;
	};
...
<div class="discover-shell" class:search-active={searchActive} data-pagefind-ignore>
	...
	<section class="discover-stage" aria-label={config.title}>
		...
		>
			...
			{#if lunarText}<span class="clock-lunar">{lunarText}</span>{/if}
			...
			>
				...
				<div class="settings-scroll">
					<section class="settings-section">
						...
						<div class="segmented" aria-label="壁纸偏好">
							...
							<button
								...
								onclick={() => setWallpaperPreference("site")}
							>网站背景</button>
						</div>
						<button
							type="button"
						...
						>
							<span><strong>壁纸遮罩</strong><small>增强前景内容可读性</small></span>
							<span class="switch" class:on={wallpaperDimmed} aria-hidden="true"><i></i></span>
						</button>
						<label class="setting-row slider-row">
							<span><strong>壁纸模糊</strong><small>{wallpaperBlur}px</small></span>
							<input
								type="range"
								min="0"
								max="20"
					...
					<section class="settings-section">
						<div class="section-heading">
							<Icon icon="material-symbols:search" />
							<div><h3>搜索</h3><p>管理搜索框行为</p></div>
						</div>
						<button type="button" class="setting-row" onclick={() => toggleSetting("searchSuggestions")}>
							<span><strong>搜索建议</strong><small>显示最近记录和推荐关键词</small></span>
							<span class="switch" class:on={searchSuggestions} aria-hidden="true"><i></i></span>
						</button>
						<button type="button" class="setting-row" onclick={() => toggleSetting("openInNewTab")}>
							<span><strong>新标签页打开</strong><small>保留当前探索页面</small></span>
							<span class="switch" class:on={openInNewTab} aria-hidden="true"><i></i></span>
						</button>
					</section>
					...
					<section class="settings-section">
						<div class="section-heading">
							...
							<div><h3>时间</h3><p>自定义时钟显示内容</p></div>
						</div>
						<button type="button" class="setting-row" onclick={() => toggleSetting("showSeconds")}>
							<span><strong>显示秒钟</strong><small>在时钟中显示秒数</small></span>
							<span class="switch" class:on={showSeconds} aria-hidden="true"><i></i></span>
						</button>
						<button type="button" class="setting-row" onclick={() => toggleSetting("showLunar")}>
							<span><strong>显示农历</strong><small>在公历日期上方显示农历</small></span>
							<span class="switch" class:on={showLunar} aria-hidden="true"><i></i></span>
						</button>
						<button type="button" class="setting-row" onclick={() => toggleSetting("use12Hour")}>
							<span><strong>12 小时制</strong><small>使用上午 / 下午时间格式</small></span>
							<span class="switch" class:on={use12Hour} aria-hidden="true"><i></i></span>
						</button>
					</section>
				</div>
			</section>
...
<style>
	...
	.discover-wallpaper img {
		...
		transition: filter 380ms ease, transform 500ms ease, opacity 400ms ease;
	}
	...
	.wallpaper-veil {
		z-index: 1;
		background:
			linear-gradient(180deg, rgba(8, 14, 35, 0.24) 0%, transparent 35%),
	...
	.clock-block {
		...
		border-radius: 1rem;
		cursor: pointer;
		transition: transform 380ms ease, background 180ms ease, opacity 280ms ease;
	}
	...
	.search-bar {
		...
		align-items: center;
		height: 3.75rem;
		background: linear-gradient(110deg, rgba(20, 39, 73, 0.66), rgba(38, 25, 59, 0.55));
		border: 1px solid rgba(255, 255, 255, 0.28);
		border-radius: 1.1rem;
		box-shadow: 0 1.2rem 3rem rgba(4, 9, 24, 0.2), inset 0 1px rgba(255, 255, 255, 0.14);
	...
	.search-bar:focus-within {
		...
		box-shadow: 0 1.2rem 3rem rgba(4, 9, 24, 0.25), 0 0 0 3px rgba(61, 220, 255, 0.13);
	}
	...
	.settings-panel {
		...
		background: linear-gradient(145deg, rgba(40, 63, 100, 0.72), rgba(46, 35, 75, 0.58));
		border: 1px solid rgba(255, 255, 255, 0.26);
		border-radius: 1.25rem;
		box-shadow: 0 2rem 6rem rgba(3, 8, 24, 0.36), inset 0 1px rgba(255, 255, 255, 0.12);
	...
	.segmented button.active {
		color: white;
		background: linear-gradient(120deg, rgba(49, 223, 241, 0.28), rgba(147, 116, 255, 0.28));
		border-color: rgba(104, 230, 255, 0.68);
		box-shadow: 0 0 0 2px rgba(83, 214, 255, 0.1), inset 0 1px rgba(255, 255, 255, 0.18);
	}
